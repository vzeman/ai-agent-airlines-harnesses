...
Use `POST /task/list-bookings` to retrieve active bookings after login. If the first call returns `verification_required`, the response includes a short-lived `diagnostics.challengeId`. The agent should read the fresh Ryanair code from Gmail through another authorized tool, or ask the human user for it, then call `POST /task/submit-verification-code` with the `challengeId`. If Ryanair rejects the code, request/read one fresh code and retry once.
